    ...
    <>
      <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
        <div className="relative w-auto my-6 mx-auto max-w-3xl">
          {/*content*/}
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-gray-100	 bg-opacity-95	outline-none focus:outline-none">
            {/*header*/}
            ...
            <div className="">
              ...
              {fields.map((field, idx) => {
                return (
                  <div
                    className="grid grid-cols-6 grid-rows-3 p-5 grid-flow-row "
                    key={`${field}-${idx}`}
                  >
                    <div key={`${field}-${idx}`} className="col-span-2 px-4 self-center">
                    <p className="pl-1">Location</p>
                      <Select
                        options={options}
                        key={`${field}-${idx}`}
                        onChange={(selected) => handleChange(idx, selected.value, "Location")}
                      />
                    </div>
                    ...
                    <p className="pl-1">Time-In</p>
                      <input
                        type="time"
                        placeholder="Time-In"
                        onChange={(selected) => handleChange(idx, selected.target.value, "Time-In")}
                    ...
                    <FontAwesomeIcon className="h-7 mt-5 text-black cursor-pointer leading-none border border-solid border-transparent rounded bg-transparent block outline-none focus:outline-none justify-self-center self-center" icon={faArrowRight}/>
                    <div className=" justify-self-center self-center">
                      <p className="pl-1">Time-Out</p>
                      <input
                        type="time"
                        placeholder="Time-Out"
                        onChange={(selected) => handleChange(idx, selected.target.value, "Time-Out")}
                    ...
                    <button className="justify-self-center self-center" onClick={() => handleRemove(idx)}>
                      <FontAwesomeIcon className="h-7 text-black cursor-pointer leading-none pr-3 border border-solid border-transparent rounded bg-transparent block outline-none focus:outline-none " icon={faTimesCircle}/>
                    </button>
                    <div className="col-span-6 ml-4 mr-4 my-3 row-span-2">
                      <input
                        type="text"
                        placeholder="Description"
                        onChange={(selected) => handleChange(idx, selected.target.value, "Description")}
              ...
              <button className="mx-auto my-4" onClick={() => handleAdd()}>
                <FontAwesomeIcon className="h-8 text-black cursor-pointer leading-none border border-solid border-transparent rounded bg-transparent block outline-none focus:outline-none" icon={faPlus}/>
              </button>
              </div>
            </div>
            {/*footer*/}
